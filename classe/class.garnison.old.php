    ...
    function appliquerDegat(){
      ...
      if($this->degat_att > 0){
        ...
        foreach($this->unite as $key => $value){
          if($value > 0){
            for($x=1;$x<=$value;$x++){
              if($GLOBALS['Aunite'][$key] < $this->degat_att){
                $this->degat_att = $this->degat_att - $GLOBALS['Aunite'][$key];
      ...
      if($this->degat_def > 0){
        ...
        foreach($this->unite as $key => $value){
          if($value > 0){
            for($x=1;$x<=$value;$x++){
              if($GLOBALS['Dunite'][$key] < $this->degat_def){
                $this->degat_def = $this->degat_def - $GLOBALS['Dunite'][$key];
                $this->SortirUnite($key,1);
              }
            }
          }
