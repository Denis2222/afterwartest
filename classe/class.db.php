<?php
  /** A PHP class to access MySQL database with convenient methods
    ...
    */
    ...
    //-----------------------------------------------------------------------
    // /!\ décommenter la ligne 51 pour afficher les requettes effectuées /!\
    //-----------------------------------------------------------------------
  ...
  class DB
  {
    ...
    function query($query, $debug = -1)
    {
      ...
      $this->nbQueries++;
      mysql_query("SET NAMES 'utf8'");// or $this->debugAndDie($query);
      $atime = $this->microtime_float();
      $this->lastResult = mysql_query($query)or $this->debugAndDie($query); // or $this->debugAndDie($query);
      $ftime = $this->microtime_float();
      $time = $ftime-$atime;
      $this->dbTime+=$time;
      //echo '<br />'.$time.' reg:'.$query;
